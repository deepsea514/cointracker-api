...
export const getTokenSwaps = async (
  ...
  const stablePair = await web3Helper.getPairAddress(address, chain.tokens.STABLE, contract)
  const nativePair = await web3Helper.getPairAddress(address, chain.tokens.NATIVE, contract)
  let pairs: string[] = []
