...
import {
  fetchTokenHistoricalDataBetweenTimeStamps,
  fetchTokenHistoricalDataBetweenTimeStampsV3,
} from '../../utils/subgraph/dataFetchHelper'
...
import { formatToken, formatTokenV3 } from '../../utils/formatters'
...
export const getTokenByAddress = async (
  ...
  pair = compareAddress(address, chain.tokens.NATIVE, chain.web3)
    ? stableNativePair?.pairAddress ??
      (await web3Helper.getPairAddress(chain.tokens.STABLE, chain.tokens.NATIVE, contract))
    : tokenNativePair?.pairAddress ?? (await web3Helper.getPairAddress(address, chain.tokens.NATIVE, contract))
...
export const getTokenByAddressV3 = async (
  chainId: CHAINS,
  exchange: EXCHANGES,
  address: string,
  cache: boolean = true,
  ...
  pair = compareAddress(address, chain.tokens.NATIVE, chain.web3)
    ? stableNativePair?.pairAddress ??
      (await web3Helper.getPairAddressV3(chain.tokens.STABLE, chain.tokens.NATIVE, contract))
    : tokenNativePair?.pairAddress ?? (await web3Helper.getPairAddressV3(address, chain.tokens.NATIVE, contract))
  ...
  const token = await formatTokenV3({
    ...tokenData,
    chain,
    exchange: exchangeDetails.name,
  })
...
export const getTokensV3 = async (chainId: CHAINS, exchange: EXCHANGES, limit: number, cache: boolean = true) => {
  const subgraph = SUBGRAPHS[`${chainId}`]?.[`${exchange}`]
  ...
  const tokens = await getOrSetCache(
    ...
    async () => {
      ...
      return await Promise.all(
        data.tokens.map((token: any) => {
          // console.log(token.address)
          return getTokenByAddressV3(chainId, exchange, token.address, cache)
        }),
      )
    },
    cache,
...
function getCandlestickFromSwaps(
  ...
  const formattedSwaps = calculateReservesAndPrice(pair, swaps, initialReserves, token0IsNative)
...
function getCandlestickFromSwapsV3(
  pair: IRawPairData,
  ...
  const formattedSwaps = calculateReservesAndPriceV3(pair, swaps, initialReserves, token0IsNative)
  if (!formattedSwaps.length) return []
  const [start, end] = calculateTimeSlots(formattedSwaps[0].timestamp, timeFrameSeconds)
  const prices = chunkIntervalSwapData(start, end, formattedSwaps, timeFrameSeconds)
  return getCandlestickData(prices, token0IsNative, chain, exchange, tokenId)
...
async function getRecentCandlesV3(
  chainId: CHAINS,
  exchange: EXCHANGES,
  chain: IChainConfiguration,
  factory: string,
  ...
  const data = await getOrSetCache(
    `tokens/historical?chainId=${chainId}&exchange=${exchange}&from=${startTime}&to=${endTime}&token0=${token0}&token1=${token1}`,
    async () => {
      const baseData = await fetchTokenHistoricalDataBetweenTimeStampsV3(
        token0,
        token1,
        subgraph,
        chain,
  ...
  return getCandlestickFromSwapsV3(
    data.pair,
    data.swaps,
    initialReserves,
    timeFrameSeconds,
...
async function nativeStableCandlesV3(
  isNativeToken: boolean,
  exchangeDetails: any,
  chain: IChainConfiguration,
  subgraph: any,
  ...
  console.log(`${chain.chainId}/${exchangeDetails.name}: Fetching ${isNativeToken ? 'Native' : 'Stable'} history`)
  // Normalize from to dates to blockchain format
  const baseCandles = await getRecentCandlesV3(
    chain.chainId,
    exchangeDetails.name,
    chain,
    exchangeDetails.address,
...
export const getTokenHistoricalV3 = async (
  chainId: CHAINS,
  exchange: EXCHANGES | null,
  timeFrameSeconds: number,
  address: string,
  ...
  // Retrieve candles for Address/Native pair
  // Don't cache
  const candleRecent = await getRecentCandlesV3(
    chainId,
    exchangeDetails.name,
    chain,
    exchangeDetails.address,
...
export const getExchange = (exchange: EXCHANGES, chain: CHAINS) => {
  ...
  switch (exchange) {
    ...
    case EXCHANGES.FUSION_DEX_V3:
      return result + '_FUSION_DEX_V3'
    case EXCHANGES.PULSE_DEX:
      return result + '_PULSE_DEX'
    case EXCHANGES.PULSE_DEX2:
      return result + '_PULSE_DEX2'
