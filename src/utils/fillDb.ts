...
import {
  ...
  getExchange,
  getTokensWithoutSevenDayData,
  getTokens,
  getTokensV3,
} from '../controllers/helpers/tokens'
...
export const fillDbWithTokensV3 = async (ch: CHAINS, ex: EXCHANGES) => {
  if (!fs.existsSync(path.join(process.cwd(), '.logs'))) fs.mkdirSync(path.join(process.cwd(), '.logs'))
  // if (ch != CHAINS.FTM) return
  try {
    const subgraph = SUBGRAPHS[ch]?.[ex]
    ...
    do {
      // Get top 100 tokens from graphql
      try {
        fetchedTokens = await getTokensV3(ch as CHAINS, ex as EXCHANGES, 100, false) // don't cache this request
        shouldRetry = false
      } catch (err: any) {
        shouldRetry = ++retries < MAX_RETRIES ? true : false
    ...
    for (let i = 0; i < newTokens.length; i++) {
      ...
      do {
        ...
        try {
          const tokenHistory = await fetchTokenHistoryV3(newTokens[i], ch, ex, from, to)
...
const fetchTokenHistoryV3 = async (
  token: IToken,
  ch: CHAINS,
  ex: EXCHANGES,
  from: number,
  ...
  return getTokenHistoricalV3(ch as CHAINS, ex, resolution, token.address, from, to, false)
}
